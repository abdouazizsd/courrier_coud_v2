                        ...
                        <tbody>
                            <?php foreach ($suivis as $suivi): ?>
                                <tr>
                                    ...
                                    <td class="text-center pt-4">
                                        <a href="#" class="btn btn-danger" onclick="confirmDeletion('<?= htmlspecialchars($suivi['id_suivi']); ?>', '<?= $_SESSION['subrole']; ?>', '<?= $_SESSION['Fonction']; ?>')">
    ...
    function confirmDeletion(idsT, func, role) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "../../../traitement/deleteTache.php?idsT=" + idsT + "&func=" + func + "&role=" + role;
            }
        });
    }
    </script>
