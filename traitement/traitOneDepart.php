...
function getCourriersByRole($search = '', $page = 1, $results_per_page = 10) {
    ...
    $query = "SELECT DISTINCT c.Numero_Courrier, c.Date, c.Objet, c.Nature, c.pdf, i.id_imputation, i.Instruction, i.date_impu , i.instruction_personnalisee
              FROM courrier c
              INNER JOIN imputation i ON c.id_courrier = i.id_courrier
    ...
    // Préparer et exécuter la requête
    $stmt = $connexion->prepare($query);
    $stmt->bind_param("sssssii", $role, $searchTerm, $searchTerm, $searchTerm , $searchTerm, $results_per_page, $offset);
    $stmt->execute();
    $result = $stmt->get_result();
